...
int crcverify(const unsigned char* message, unsigned int length)
{
  ...
  return (expected & 0xff) == (~reverse(message[length - 2]) & 0xFF) &&
    ((expected >> 8) & 0xff) == (~reverse(message[length - 1]) & 0xFF);
}
...
void crcappend(unsigned char* message, unsigned int length)
{
  ...
  message[length] = ~reverse((unsigned char)(crc & 0xff));
  message[length+1] = ~reverse((unsigned char)((crc >> 8) & 0xff));
}
